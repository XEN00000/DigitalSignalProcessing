...
class SignalApp(tk.Tk):
    ...
    def generate_and_plot(self):
        """Metoda wywoływana po kliknięciu 'Generuj'."""
        try:
            ...
            # 2. Pobranie specyficznych parametrów i instancjonowanie odpowiedniej klasy
            if signal_type == "Szum Gaussowski":
                # Szum nie wymaga dodatkowych parametrów z Entry
            ...
            elif signal_type in ["Sygnał Sinusoidalny", "Sygnał Sinusoidalny wyprostowany jednopołówkowo", "Sygnał Sinusoidalny wyprostowany dwupołówkowo"]:
                ...
                if signal_type == "Sygnał Sinusoidalny":
                    ...
                    generator = HalfWaveSineGenerator(
                        A, t1, d, f, sig_f, phase)
                else:
                    generator = FullWaveSineGenerator(
                        A, t1, d, f, sig_f, phase)
                ...
                # ns to liczba całkowita (numer próbki)
            ...
            # 4. Opakowanie wygenerowanych danych w obiekt core.Signal (wymagane przez FileHandler)
            # Upewnij się, że zaimportowałeś: from core.Signal import Signal
            self.current_signal = Signal()
            self.current_signal.t1 = t1
            self.current_signal.f = f
            self.current_signal.is_complex = False
            self.current_signal.num_samples = len(y)
            self.current_signal.amplitudes = y
            self.current_signal.time_axis = t

            # (Opcjonalnie) Zapisujemy informację, czy sygnał jest dyskretny (przydatne przy rysowaniu)
            self.current_signal.is_discrete = signal_type in [
                "Szum Impulsowy", "Impuls jednostkowy"]

            # 5. Aktualizacja starych zmiennych z GUI dla wygody
            self.current_signal_time = t
            self.current_signal_values = y

            # 6. Obliczenie parametrów sygnału (średnia, RMS itp.) i odrysowanie wykresów
            self.calculate_parameters()
            self.update_plots()

        except ValueError:
            messagebox.showerror(
                "Błąd typu danych", "Proszę upewnić się, że wszystkie wpisane parametry są poprawnymi liczbami (np. 1.5, a nie 1,5).")
        except Exception as e:
            messagebox.showerror(
                "Błąd niespodziewany", f"Wystąpił problem podczas generowania sygnału:\n{e}")
